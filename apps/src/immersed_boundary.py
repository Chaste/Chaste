...
def pvd_to_mp4(sim_dir):
    ...
    for dir in os.listdir(sim_dir):
        if dir.startswith('results_from_time'):
            possible_data_directories.append(sim_dir + dir + '/')
    ...
    # This parameter sets the 'zoom' and needs fine-tuning: however, 0.5 seems perfect to capture exactly the unit square
    ...
    pv.WriteAnimation(sim_dir + 'results.png', Magnification=1, Compression=False)

    # Raise exception if the png files are not generated as expected
    if not(os.path.isfile(sim_dir + 'results.0000.png')):
