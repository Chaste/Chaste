...
#include "MathsCustomFunctions.hpp"
    ...
    void CellNobleVargheseKohlNoble1998aFromCellMLCvodeOpt::EvaluateYDerivatives(double var_chaste_interface__environment__time_converted, const N_Vector rY, N_Vector rDY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : NV_Ith_S(rY, 0));
    ...
    void CellNobleVargheseKohlNoble1998aFromCellMLCvodeOpt::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : NV_Ith_S(rY, 0));
        // Units: millivolt; Initial value: -92.849333
        double var_chaste_interface__rapid_delayed_rectifier_potassium_current_xr1_gate__xr1 = NV_Ith_S(rY, 1);
        ...
        const double var_x111 = exp(-3.6960000000000002 - 0.056000000000000001 * var_chaste_interface__membrane__V);
        const double var_x112 = 0.44800000000000001 * var_x111 * var_chaste_interface__fast_sodium_current_m_gate__m;
        const double var_x113 = 41.0 + var_chaste_interface__membrane__V;
        const double var_x114 = fabs(var_x113) < 1.0000000000000001e-5;
        const double var_x115 = 1.0 - var_chaste_interface__fast_sodium_current_m_gate__m;
        ...
        const double var_x119 = 0.20000000000000001 / var_x118;
        const double var_x120 = -8.0 * var_x111;
        const double var_x121 = exp(-9.375 + var_x0);
        const double var_x122 = exp(-7.5 + var_x116);
        const double var_x123 = 19.0 + var_chaste_interface__membrane__V;
        const double var_x124 = fabs(var_x123) < 0.0001;
        const double var_x125 = 1.0 - var_chaste_interface__L_type_Ca_channel_d_gate__d;
        const double var_x126 = 0.25 * var_chaste_interface__membrane__V;
        const double var_x127 = -var_x126;
        const double var_x128 = exp(-4.75 + var_x127);
        const double var_x129 = 1.0 - var_x128;
        const double var_x130 = 0.089999999999999997 / var_x129;
        const double var_x131 = exp(1.8999999999999999 + 0.10000000000000001 * var_chaste_interface__membrane__V);
        const double var_x132 = -1.0 + var_x131;
        const double var_x133 = 0.035999999999999997 / var_x132;
        const double var_x134 = exp(-8.5 + var_x127);
        const double var_x135 = 1.0 + var_x134;
        const double var_x136 = -0.00089999999999999998 * var_x134 * var_chaste_interface__L_type_Ca_channel_f_gate__f / pow(var_x135, 2);
        const double var_x137 = 34.0 + var_chaste_interface__membrane__V;
        const double var_x138 = fabs(var_x137) < 0.0001;
        const double var_x139 = 1.0 - var_chaste_interface__L_type_Ca_channel_f_gate__f;
        const double var_x140 = exp(8.5 + var_x126);
        const double var_x141 = -1.0 + var_x140;
        const double var_x142 = 0.0018749999999999999 / var_x141;
        const double var_x143 = -0.0035999999999999999 / var_x135;
        const double var_x144 = 0.001 + var_chaste_interface__intracellular_calcium_concentration__Ca_ds;
        const double var_x145 = exp(-0.058823529411764705 * var_chaste_interface__membrane__V);
        const double var_x146 = exp(-1.25 + var_x0);
        const double var_x147 = 1.0 + var_x146;
        const double var_x148 = exp(-0.80000000000000004 - 0.20000000000000001 * var_chaste_interface__membrane__V);
        const double var_x149 = 0.00050000000000000001 + var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        const double var_x150 = 1 / var_x149;
        const double var_x151 = var_x150 * var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        const double var_x152 = 0.01 + var_chaste_interface__intracellular_calcium_concentration__Ca_ds;
        const double var_x153 = 1 / var_x152;
        const double var_x154 = 1.0 - var_x151;
        const double var_x155 = var_x153 * var_x154;
        const double var_x156 = var_x151 + var_x155 * var_chaste_interface__intracellular_calcium_concentration__Ca_ds;
        const double var_x157 = 0.5 * pow(var_x156, 2);
        const double var_x158 = ((var_chaste_interface__membrane__V < -50.0) ? (5.0) : (1.0));
        const double var_x159 = -var_x157 * var_x158;
        const double var_x160 = 1.0 - var_chaste_interface__calcium_release__ActFrac - var_chaste_interface__calcium_release__ProdFrac;
        const double var_x161 = var_chaste_interface__intracellular_calcium_concentration__Ca_i / pow(var_x149, 2);
        const double var_x162 = 2 * var_chaste_interface__intracellular_calcium_concentration__Ca_ds;
        const double var_x163 = 0.5 * var_x156 * var_x158;
        const double var_x164 = var_x163 * (-2 * var_x161 + 2 * var_x150 + var_x153 * var_x162 * (var_x161 - var_x150));
        const double var_x165 = var_x164 * var_chaste_interface__calcium_release__ActFrac;
        const double var_x166 = var_x163 * (2 * var_x155 - var_x154 * var_x162 / pow(var_x152, 2));
        const double var_x167 = var_x166 * var_chaste_interface__calcium_release__ActFrac;
        const double var_x168 = 2.3651760162759904e-8 * var_x40;
        const double var_x169 = 4.7303520325519805e-9 * var_x49;
        const double var_x170 = 8.4392437745822447e-5 * var_x6 - 3.1591373008424612e-6 * var_chaste_interface__membrane__V;
        ...
        const double var_x183 = var_x172 * var_x174;
        const double var_x184 = 100.0 * var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        const double var_x185 = 4.7303520325519803e-5 * var_x47;
        const double var_x186 = -var_x184;
        ...
        IJth(rJacobian, 4, 0) = ((var_x114) ? (var_x112) : (var_x112 + var_x115 * var_x119 - 0.02 * var_x113 * var_x115 * var_x117 / pow(var_x118, 2)));
        IJth(rJacobian, 5, 0) = -0.125 * var_x121 * (0.02 - 0.02 * var_chaste_interface__fast_sodium_current_h_gate__h) - 0.00062500000000000001 * var_x122 * var_chaste_interface__fast_sodium_current_h_gate__h / pow((0.0031250000000000002 + var_x122), 2);
        IJth(rJacobian, 6, 0) = ((var_x124) ? (0) : (var_x125 * var_x130 - var_x133 * var_chaste_interface__L_type_Ca_channel_d_gate__d + 0.0035999999999999999 * var_x123 * var_x131 * var_chaste_interface__L_type_Ca_channel_d_gate__d / pow(var_x132, 2) - 0.022499999999999999 * var_x123 * var_x125 * var_x128 / pow(var_x129, 2)));
        IJth(rJacobian, 7, 0) = ((var_x138) ? (var_x136) : (var_x136 + var_x139 * var_x142 - 0.00046874999999999998 * var_x137 * var_x139 * var_x140 / pow(var_x141, 2)));
        IJth(rJacobian, 10, 0) = -0.058823529411764705 * var_x145 * (3.3000000000000003e-5 - 3.3000000000000003e-5 * var_chaste_interface__transient_outward_current_s_gate__s) - 0.0041250000000000002 * var_x146 * var_chaste_interface__transient_outward_current_s_gate__s / pow(var_x147, 2);
        IJth(rJacobian, 11, 0) = 0.066600000000000006 * var_x148 / pow((1.0 + var_x148), 2);
        IJth(rJacobian, 14, 0) = -3.7909647610109532e-7 + 8.8537740769851485e-10 * var_x60 - 9.4679344906248566e-7 * var_x25 - 1.2395283707779208e-7 * var_x56 - 0.0015795686504212305 * var_x16 - 3.1591373008424611e-7 * var_x29 - 2.5273098406739689e-6 * var_x3 - var_x168 * var_x38;
        IJth(rJacobian, 15, 0) = 1.7707548153970298e-10 * var_x59 + 2.8206583043236264e-5 * var_x32 + 4.2235286004928392e-6 * var_x31 - 7.0830192615881192e-10 * var_x56 - 3.1591373008424612e-6 * var_x17 - 0.00063182746016849224 * var_x14 - 9.0261065738356038e-5 * var_x9 - 1.6427513964380798e-6 * var_x4 - var_x169 * var_x38;
        ...
        IJth(rJacobian, 4, 4) = ((var_x114) ? (-2.0 + var_x120) : (var_x120 - var_x113 * var_x119));
        ...
        IJth(rJacobian, 5, 5) = -2.0 / (1.0 + 320.0 * var_x122) - 0.02 * var_x121;
        ...
        IJth(rJacobian, 6, 6) = ((var_x124) ? (-0.71999999999999997) : (-var_x123 * var_x130 - var_x123 * var_x133));
        IJth(rJacobian, 14, 6) = -var_x168 * var_x72;
        IJth(rJacobian, 15, 6) = -var_x169 * var_x72;
        IJth(rJacobian, 17, 6) = -var_x185 * var_x71;
        IJth(rJacobian, 0, 7) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x62 * (var_x41 * var_x75 + var_x48 * var_x74 + var_x50 * var_x75));
        IJth(rJacobian, 7, 7) = ((var_x138) ? (-0.0074999999999999997 + var_x143) : (var_x143 - var_x137 * var_x142));
        IJth(rJacobian, 14, 7) = -var_x168 * var_x75;
        IJth(rJacobian, 15, 7) = -var_x169 * var_x75;
        IJth(rJacobian, 17, 7) = -var_x185 * var_x74;
        ...
        IJth(rJacobian, 14, 9) = -var_x168 * var_x77;
        IJth(rJacobian, 15, 9) = -var_x169 * var_x77;
        IJth(rJacobian, 17, 9) = -var_x185 * var_x76;
        IJth(rJacobian, 0, 10) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x78 * var_chaste_interface__transient_outward_current_r_gate__r);
        IJth(rJacobian, 10, 10) = -0.033000000000000002 / var_x147 - 3.3000000000000003e-5 * var_x145;
        IJth(rJacobian, 15, 10) = var_x170 * var_chaste_interface__transient_outward_current_r_gate__r;
        ...
        IJth(rJacobian, 15, 11) = var_x170 * var_chaste_interface__transient_outward_current_s_gate__s;
        IJth(rJacobian, 12, 12) = var_x159 + var_x158 * (-0.059999999999999998 - var_x157);
        IJth(rJacobian, 13, 12) = var_x158 * (0.059999999999999998 + var_x157);
        IJth(rJacobian, 16, 12) = (0.10204081632653063 * var_x173 - 0.10204081632653063 * var_x175) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        IJth(rJacobian, 19, 12) = (0.5 * var_x175 - 0.5 * var_x173) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        IJth(rJacobian, 12, 13) = var_x159;
        IJth(rJacobian, 13, 13) = -0.001 * var_x158;
        ...
        IJth(rJacobian, 14, 14) = 6.634188331769168e-7 * var_x81 - 6.7513950196657958e-5 * var_x80 - 0.0010614701330830668 * var_x79 - 1.0127092529498693e-5 * var_x28 - 5.6807606943749135e-6 * var_x85 - 2.3651760162759904e-8 * var_x87 - 5.9277248272665686 * var_x86;
        ...
        IJth(rJacobian, 15, 15) = 0.00011282633217294504 * var_x91 - 0.0024112125070234981 * var_x88 - 8.4392437745822447e-5 * var_x90 - 0.00035984935454818685 * var_x84 - 0.016878487549164489 * var_x89 - 4.7303520325519805e-9 * var_x87;
        ...
        IJth(rJacobian, 12, 16) = -var_x165 + var_x160 * var_x164;
        IJth(rJacobian, 13, 16) = var_x165;
        IJth(rJacobian, 14, 16) = 2.5980012242274606 * var_x93 + 6.5328747985311509e-9 * var_x96;
        IJth(rJacobian, 16, 16) = -7.0 + var_x176 + var_x177 + 0.00040000000000000002 * var_x181 - 0.00040000000000000002 * var_x179 - 0.86600040807582024 * var_x93 - 1.0549054718227806e-6 * var_x92 - 2.1776249328437171e-9 * var_x96 - 7.1999999999999991e-9 * var_x182;
        IJth(rJacobian, 17, 16) = -4.7303520325519803e-5 * var_x97;
        IJth(rJacobian, 18, 16) = 3.5279999999999993e-7 * var_x182 + 0.019599999999999999 * var_x179 - 0.019599999999999999 * var_x181;
        IJth(rJacobian, 20, 16) = 2.0 - var_x176;
        IJth(rJacobian, 21, 16) = 5.0 - var_x177;
        IJth(rJacobian, 0, 17) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x62 * (-3.4500000000000003e-9 * var_x98 - 1.3720000000000001 * var_x99));
        IJth(rJacobian, 9, 17) = -0.02 / var_x144 + 0.02 * var_chaste_interface__intracellular_calcium_concentration__Ca_ds / pow(var_x144, 2);
        IJth(rJacobian, 12, 17) = -var_x167 + var_x160 * var_x166;
        IJth(rJacobian, 13, 17) = var_x167;
        IJth(rJacobian, 16, 17) = 0.001 - 0.00086686727535117134 * var_x99 - 2.1798047375812983e-12 * var_x98;
        IJth(rJacobian, 17, 17) = -0.01;
        IJth(rJacobian, 16, 18) = 9.5999999999999999e-8 * var_x181 + 7.1999999999999991e-9 * var_x179 - 1.7279999999999999e-12 * var_x182;
        IJth(rJacobian, 18, 18) = -0.050000000000000003 + 8.4671999999999982e-11 * var_x182 - 4.7039999999999993e-6 * var_x181 - 3.5279999999999993e-7 * var_x179;
        IJth(rJacobian, 19, 18) = 0.0050000000000000001;
        IJth(rJacobian, 16, 19) = 1.0204081632653063e-5 + 0.051020408163265314 * var_x183;
        IJth(rJacobian, 18, 19) = 0.050000000000000003;
        IJth(rJacobian, 19, 19) = -0.0050499999999999998 - 0.25 * var_x183;
        IJth(rJacobian, 16, 20) = 0.050000000000000003 + var_x184;
        IJth(rJacobian, 20, 20) = -0.050000000000000003 + var_x186;
        IJth(rJacobian, 16, 21) = 0.20000000000000001 + var_x184;
        IJth(rJacobian, 21, 21) = -0.20000000000000001 + var_x186;
    }

    N_Vector CellNobleVargheseKohlNoble1998aFromCellMLCvodeOpt::ComputeDerivedQuantities(double var_chaste_interface__environment__time_converted, const N_Vector & rY)
    {
        // Inputs:
        // Time units: millisecond
