...
- [ ]  Add the new docker image to the [portability workflow](https://github.com/Chaste/Chaste/blob/develop/.github/workflows/tier-2-portability.yml) on the new branch.
- [ ]  Create a PR with the new branch and see if it builds and passes all tests.
- [ ]  Do any necessary fixes.
- [ ]  Add permanently to testing rotation i.e. either replace an old docker image in the [portability workflow](https://github.com/Chaste/Chaste/blob/develop/.github/workflows/tier-2-portability.yml) with the new image, or append the new image to the existing list of portability images. For example, if there's an old dependency version that is being retired, the new docker image can probably replace the old one.
